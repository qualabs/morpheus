...
} ngx_http_morpheus_loc_conf_t;


typedef struct {
    ngx_str_t   path;
    size_t      len;
} ngx_http_morpheus_copy_ctx_t;


static ngx_int_t ngx_http_morpheus_handler(ngx_http_request_t *r);

static void ngx_http_morpheus_put_handler(ngx_http_request_t *r);

static ngx_int_t ngx_http_morpheus_delete_handler(ngx_http_request_t *r);
static ngx_int_t ngx_http_morpheus_delete_path(ngx_http_request_t *r,
    ngx_str_t *path, ngx_uint_t dir);
static ngx_int_t ngx_http_morpheus_delete_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path);
static ngx_int_t ngx_http_morpheus_delete_file(ngx_tree_ctx_t *ctx, ngx_str_t *path);
static ngx_int_t ngx_http_morpheus_noop(ngx_tree_ctx_t *ctx, ngx_str_t *path);

static ngx_int_t ngx_http_morpheus_mkcol_handler(ngx_http_request_t *r,
    ngx_http_morpheus_loc_conf_t *dlcf);

static ngx_int_t ngx_http_morpheus_copy_move_handler(ngx_http_request_t *r);
static ngx_int_t ngx_http_morpheus_copy_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path);
static ngx_int_t ngx_http_morpheus_copy_dir_time(ngx_tree_ctx_t *ctx,
    ngx_str_t *path);
static ngx_int_t ngx_http_morpheus_copy_tree_file(ngx_tree_ctx_t *ctx,
    ngx_str_t *path);

static ngx_int_t ngx_http_morpheus_depth(ngx_http_request_t *r, ngx_int_t dflt);
static ngx_int_t ngx_http_morpheus_error(ngx_log_t *log, ngx_err_t err,
    ngx_int_t not_found, char *failed, u_char *path);
static ngx_int_t ngx_http_morpheus_location(ngx_http_request_t *r);
static void *ngx_http_morpheus_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_morpheus_merge_loc_conf(ngx_conf_t *cf,
...
void morph_process(const char* td, const char* dc);
...
static ngx_command_t ngx_http_morpheus_commands[] = {

    { ngx_string("morpheus"), /* directive */
      NGX_HTTP_LOC_CONF|NGX_CONF_NOARGS, /* location context and takes no arguments*/
      ngx_http_morpheus,      /* configuration setup function */
      0,                      /* No offset. Only one context is supported. */
      0,                      /* No offset when storing the module configuration on struct. */
      NULL},
 
    { ngx_string("morph_dav_methods"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
      ngx_conf_set_bitmask_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_morpheus_loc_conf_t, methods),
      &ngx_http_morpheus_methods_mask },

    { ngx_string("morph_create_full_put_path"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_morpheus_loc_conf_t, create_full_put_path),
      NULL },

    { ngx_string("morph_min_delete_depth"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_morpheus_loc_conf_t, min_delete_depth),
      NULL },

    { ngx_string("morph_dav_access"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE123,
      ngx_conf_set_access_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_morpheus_loc_conf_t, access),
      NULL },
...
static ngx_http_module_t  ngx_http_morpheus_module_ctx = {
    NULL,                                  /* preconfiguration */
    ngx_http_morpheus_init,                /* postconfiguration */
    ...
    ngx_http_morpheus_create_loc_conf,     /* create location configuration */
    ngx_http_morpheus_merge_loc_conf       /* merge location configuration */
};


ngx_module_t  ngx_http_morpheus_module = {
    NGX_MODULE_V1,
    &ngx_http_morpheus_module_ctx,         /* module context */
    ngx_http_morpheus_commands,            /* module directives */
...
static char *ngx_http_morpheus(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /* Install the morpheus handler. */
    clcf = (ngx_http_core_loc_conf_t*)ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_morpheus_handler;
...
static ngx_int_t
ngx_http_morpheus_handler(ngx_http_request_t *r)
{
    ngx_int_t                 rc;
    ngx_http_morpheus_loc_conf_t  *dlcf;

    dlcf = (ngx_http_morpheus_loc_conf_t*)ngx_http_get_module_loc_conf(r, ngx_http_morpheus_module);
    ...
    case NGX_HTTP_PUT:
        ...
        rc = ngx_http_read_client_request_body(r, ngx_http_morpheus_put_handler);
    ...
    case NGX_HTTP_DELETE:
        ...
        return ngx_http_morpheus_delete_handler(r);

    case NGX_HTTP_MKCOL:

        return ngx_http_morpheus_mkcol_handler(r, dlcf);

    case NGX_HTTP_COPY:

        return ngx_http_morpheus_copy_move_handler(r);
...
static void
ngx_http_morpheus_put_handler(ngx_http_request_t *r)
{
    size_t                    root;
    time_t                    date;
    ngx_str_t                *temp, path;
   ...
   */
    ...
    morph_process((const char*)temp->data, NULL);
    ...
    if (status == NGX_HTTP_CREATED) {
        if (ngx_http_morpheus_location(r) != NGX_OK) {
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
...
static ngx_int_t
ngx_http_morpheus_delete_handler(ngx_http_request_t *r)
{
    size_t                    root;
    ngx_err_t                 err;
    ngx_int_t                 rc, depth;
...
ok:
    ...
    if (ngx_link_info(path.data, &fi) == NGX_FILE_ERROR) {
        ...
        return ngx_http_morpheus_error(r->connection->log, err,
    ...
    if (ngx_is_dir(&fi)) {
        ...
        depth = ngx_http_morpheus_depth(r, NGX_HTTP_DAV_INFINITY_DEPTH);
        ...
        depth = ngx_http_morpheus_depth(r, 0);
    ...
    rc = ngx_http_morpheus_delete_path(r, &path, dir);
...
static ngx_int_t
ngx_http_morpheus_delete_path(ngx_http_request_t *r, ngx_str_t *path, ngx_uint_t dir)
{
    char            *failed;
    ngx_tree_ctx_t   tree;
    ...
    if (dir) {
        ...
        tree.file_handler = ngx_http_morpheus_delete_file;
        tree.pre_tree_handler = ngx_http_morpheus_noop;
        tree.post_tree_handler = ngx_http_morpheus_delete_dir;
        tree.spec_handler = ngx_http_morpheus_delete_file;
        tree.data = NULL;
        tree.alloc = 0;
    ...
    return ngx_http_morpheus_error(r->connection->log, ngx_errno,
                              NGX_HTTP_NOT_FOUND, failed, path->data);
}


static ngx_int_t
ngx_http_morpheus_delete_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ...
    if (ngx_delete_dir(path->data) == NGX_FILE_ERROR) {
        ...
        (void) ngx_http_morpheus_error(ctx->log, ngx_errno, 0, ngx_delete_dir_n,
                                  path->data);
    }
...
static ngx_int_t
ngx_http_morpheus_delete_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0,
    ...
    if (ngx_delete_file(path->data) == NGX_FILE_ERROR) {
        ...
        (void) ngx_http_morpheus_error(ctx->log, ngx_errno, 0, ngx_delete_file_n,
                                  path->data);
    }
...
static ngx_int_t
ngx_http_morpheus_noop(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    return NGX_OK;
}
...
static ngx_int_t
ngx_http_morpheus_mkcol_handler(ngx_http_request_t *r, ngx_http_morpheus_loc_conf_t *dlcf)
{
    u_char    *p;
    size_t     root;
    ngx_str_t  path;
...
static ngx_int_t
ngx_http_morpheus_copy_move_handler(ngx_http_request_t *r)
{
    ...
    ngx_ext_rename_file_t     ext;
    ngx_http_morpheus_copy_ctx_t   copy;
    ngx_http_morpheus_loc_conf_t  *dlcf;
...
overwrite_done:
    ...
    if (ngx_is_dir(&fi)) {
        ...
        if (overwrite) {
            ...
            rc = ngx_http_morpheus_delete_path(r, &copy.path, dir);
    ...
    if (ngx_is_dir(&fi)) {
        ...
        tree.init_handler = NULL;
        tree.file_handler = ngx_http_morpheus_copy_tree_file;
        tree.pre_tree_handler = ngx_http_morpheus_copy_dir;
        tree.post_tree_handler = ngx_http_morpheus_copy_dir_time;
        tree.spec_handler = ngx_http_morpheus_noop;
        tree.data = &copy;
        tree.alloc = 0;
        tree.log = r->connection->log;
        ...
        if (ngx_walk_tree(&tree, &path) == NGX_OK) {
            ...
            if (r->method == NGX_HTTP_MOVE) {
                rc = ngx_http_morpheus_delete_path(r, &path, 1);
...
static ngx_int_t
ngx_http_morpheus_copy_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    u_char                   *p, *dir;
    size_t                    len;
    ngx_http_morpheus_copy_ctx_t  *copy;
    ...
    copy = (ngx_http_morpheus_copy_ctx_t*)ctx->data;
    ...
    if (ngx_create_dir(dir, ngx_dir_access(ctx->access)) == NGX_FILE_ERROR) {
        (void) ngx_http_morpheus_error(ctx->log, ngx_errno, 0, ngx_create_dir_n,
                                  dir);
    }
...
static ngx_int_t
ngx_http_morpheus_copy_dir_time(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    u_char                   *p, *dir;
    size_t                    len;
    ngx_http_morpheus_copy_ctx_t  *copy;
    ...
    if (fd == NGX_INVALID_FILE) {
        (void) ngx_http_morpheus_error(ctx->log, ngx_errno, 0, ngx_open_file_n, dir);
        goto failed;
    }
...
static ngx_int_t
ngx_http_morpheus_copy_tree_file(ngx_tree_ctx_t *ctx, ngx_str_t *path)
{
    u_char                   *p, *file;
    size_t                    len;
    ngx_copy_file_t           cf;
...
static ngx_int_t
ngx_http_morpheus_depth(ngx_http_request_t *r, ngx_int_t dflt)
{
...
static ngx_int_t
ngx_http_morpheus_error(ngx_log_t *log, ngx_err_t err, ngx_int_t not_found,
...
static ngx_int_t
ngx_http_morpheus_location(ngx_http_request_t *r)
{
...
static void *
ngx_http_morpheus_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_morpheus_loc_conf_t  *conf;

    conf = (ngx_http_morpheus_loc_conf_t*)ngx_pcalloc(cf->pool, sizeof(ngx_http_morpheus_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_morpheus_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_morpheus_loc_conf_t  *prev = (ngx_http_morpheus_loc_conf_t*)parent;
    ngx_http_morpheus_loc_conf_t  *conf = (ngx_http_morpheus_loc_conf_t*)child;
...
static ngx_int_t
ngx_http_morpheus_init(ngx_conf_t *cf)
{
